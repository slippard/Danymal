...
describe("/api/users tests", () => {
  ...
  const registerNewUser = (): request.Test => {
    return request(app)
      ...
      .send({ email: "danny@dannylee.co", password: "demo-password" });
  };
  ...
  it("should return invalid credentials error when login is invalid", async () => {
    ...
    const response = await request(app)
      .post("/api/users/login")
      .send({ email: "danny@dannylee.co", password: "wrong-password" });
    expect(response.status).toBe(400);
  ...
  it("should get user profile", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body.email).toBe("danny@dannylee.co");
  });
